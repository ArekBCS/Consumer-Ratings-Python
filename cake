...
def get_cake_campaign_summary():
    ...
    # Loop to get data for the past 30 days
...
def get_cake_clicks_data():
    print('Started get_cake_clicks_data')
    
    # Initialize an empty DataFrame to store Cake Conversions data
    df_cake_clicks = pd.DataFrame()
    
    end_date = datetime.now().strftime('%m-%d-%Y')  # Yesterday
    ...
    platforms = [
        ('Cake - BrandX', 'tRN2zw2xOwkLbU88jpaYoA', 573, 'https://track.brandxads.com/affiliates/api/Reports/Clicks'),
        ('Cake - Consumer Rating', 'gJJNQYtnXjJXi9Fw9ZGAxA', 3, 'https://crwebtracking.com/affiliates/api/Reports/Clicks'),
        ('Cake - Thinx', 'Vv8iFZRBFUE7gsWw8SVCQ', 9, 'https://login.thinxmg.com/affiliates/api/Reports/Clicks'),
        ('Cake - Dog Food Rankings', 'APbqDVRqAgRggstUiLlA', 621, 'https://track.brandxads.com/affiliates/api/Reports/Clicks'),
        ('Cake - Mattress Authority', 'ePNef0Asv0Nxxy5EcJw', 616, 'https://track.brandxads.com/affiliates/api/Reports/Clicks'),
        ('Cake - Top Weight Loss', 'XA1okf0lh5js7Ii0DEoniw', 619, 'https://track.brandxads.com/affiliates/api/Reports/Clicks')
    ]
    ...
    for platform_name, api_key, affiliate_id, url in platforms:
        ...
        if response.status_code == 200:
            ...
            for details in data:
                ...
                for key, value in details.items():
                    if key == 'offer':
                        row['offer_id'] = value.get('offer_id', None)
                        row['offer_name'] = value.get('offer_name', None)
                    elif key == 'redirect_from_offer':
                        row['redirect_from_offer_id'] = value.get('offer_id', None)
                        row['redirect_from_offer_name'] = value.get('offer_name', None)
                    else:
                        row[key] = value  # Dynamically add column names based on the keys
            ...
            # Convert the list of rows into a DataFrame and concatenate with the main DataFrame
            platform_df = pd.DataFrame(platform_rows)
            df_cake_clicks = pd.concat([df_cake_clicks, platform_df], ignore_index=True)
        else:
            print(f"Error: Received status code {response.status_code} from {platform_name}")

    print('Finished get_cake_clicks_data')
    return df_cake_clicks
...
cake_clicks_data = get_cake_clicks_data()

# Combine all data into one DataFrame
final_data = pd.concat([cake_campaign_summary, cake_conversions_data, cake_clicks_data], ignore_index=True)
display(final_data)

try:
    domo.write_dataframe(final_data, 'PROD_JUP_CR_CAKE')
